...
DARKWEB_CHANNEL_ID = 123456789012345678  # Sostituisci con l'ID del canale dedicato

# Dizionario per tracciare le sessioni attive: { username: discord_id }
active_darkweb_sessions = {}

def genera_password_casuale() -> str:
    caratteri = string.ascii_letters + string.digits
    return "".join(random.choices(caratteri, k=10))


# --- 1. VIEW PERSISTENTE DI ACCESSO (Da mettere nel canale di login) ---
class DarkWebLoginPersistentView(ui.View):
    def __init__(self):
        super().__init__(timeout=None)  # Persistente

    @ui.button(label="Accedi al Dark Web", style=discord.ButtonStyle.danger, emoji="🔒", custom_id="persistent_darkweb_login")
    async def accedi_btn(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_modal(DarkWebLoginModal())


# --- 2. VIEW PERSISTENTE DELLA DASHBOARD (Nel canale protetto del Dark Web) ---
class DarkWebDashboardPersistentView(ui.View):
    def __init__(self):
        super().__init__(timeout=None)  # Persistente

    @ui.button(label="Mercato Nero", style=discord.ButtonStyle.danger, emoji="🛒", custom_id="persistent_dw_mercato", row=0)
    async def mercato_nero(self, interaction: discord.Interaction, button: ui.Button):
        user_discord_id = str(interaction.user.id)
        is_connected = any(d_id == user_discord_id for d_id in active_darkweb_sessions.values())
        
        if not is_connected:
            await interaction.response.send_message("❌ Sessione scaduta o non attiva.", ephemeral=True)
            return
            
        await interaction.response.send_message("🕵️ Accesso al catalogo illegale in corso...", ephemeral=True)

    @ui.button(label="Disconnettiti (Rimuovi tracce)", style=discord.ButtonStyle.secondary, emoji="🔌", custom_id="persistent_dw_logout", row=1)
    async def disconnettiti(self, interaction: discord.Interaction, button: ui.Button):
        user = interaction.user
        user_discord_id = str(user.id)
        
        # Rimuove le sessioni attive legate all'utente
        sessions_to_remove = [u for u, d_id in active_darkweb_sessions.items() if d_id == user_discord_id]
        if not sessions_to_remove:
            await interaction.response.send_message("❌ Non risulti connesso con alcuna sessione attiva.", ephemeral=True)
            return

        for u in sessions_to_remove:
            del active_darkweb_sessions[u]
        
        # Oscura nuovamente il canale rimuovendo i permessi
        channel = interaction.guild.get_channel(DARKWEB_CHANNEL_ID)
        if channel:
            await channel.set_permissions(user, overwrite=None)
        
        await interaction.response.send_message(
            "🔌 **Connessione interrotta.** Il canale è stato oscurato e tutte le tracce locali sono state cancellate.",
            ephemeral=True
        )


# --- MODAL PER CREARE LE CREDENZIALI ---
class CreaCredenzialiModal(ui.Modal, title="Genera Credenziali Dark Web"):
    def __init__(self, target_user: discord.Member):
        super().__init__()
        self.target_user = target_user

        self.username_input = ui.TextInput(
            label="Username Anonimo",
            placeholder="Es. Shadow_Broker",
            required=True,
            max_length=50
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        password_chiara = genera_password_casuale()
        password_hash = hashlib.sha256(password_chiara.encode()).hexdigest()
        target_discord_id = str(self.target_user.id)

        existing = supabase.table("darkweb_users").select("*").eq("username", username).execute()
        
        if existing.data:
            await interaction.response.send_message(f"❌ Lo username **{username}** esiste già nel database del Dark Web!", ephemeral=True)
            return

        supabase.table("darkweb_users").insert({
        ...
        try:
            await self.target_user.send(
                f"🕵️ **Ti sono state assegnate delle credenziali per il Dark Web.**\n\n"
                f"👤 **Username:** `{username}`\n"
                f"🔑 **Password:** `{password_chiara}`\n\n"
                f"⚠️ *Conserva gelosamente questi dati. Per accedere serviranno SOLO username e password.*"
            )
            await interaction.response.send_message(
                f"✅ Credenziali generate e inviate in DM a **{self.target_user.mention}**!",
                ephemeral=True
            )
        except discord.Forbidden:
            await interaction.response.send_message(
                f"⚠️ Credenziali create, ma **non è stato possibile inviare il DM** a {self.target_user.mention}.\nUsername: `{username}` | Password: `{password_chiara}`",
                ephemeral=True
            )


@bot.tree.command(name="crea_darkweb", description="Genera credenziali Dark Web per un utente.")
@app_commands.describe(utente="L'utente a cui inviare le credenziali in DM")
async def crea_darkweb(interaction: discord.Interaction, utente: discord.Member):
    RUOLO_AUTORIZZATO_ID = 123456789012345678  # Sostituisci con l'ID del tuo ruolo

    if not any(r.id == RUOLO_AUTORIZZATO_ID for r in interaction.user.roles):
        await interaction.response.send_message("❌ Non hai i permessi per generare credenziali Dark Web.", ephemeral=True)
        return

    await interaction.response.send_modal(CreaCredenzialiModal(target_user=utente))


# --- MODAL DI LOGIN AL DARK WEB ---
class DarkWebLoginModal(ui.Modal, title="Accesso Terminale Tor"):
    def __init__(self):
        super().__init__()

        self.username = ui.TextInput(
            label="Username",
            placeholder="Il tuo username anonimo",
            required=True,
            max_length=50
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        input_user = self.username.value.strip()
        input_pass = self.password.value.strip()
        input_pass_hash = hashlib.sha256(input_pass.encode()).hexdigest()

        res = supabase.table("darkweb_users").select("*").eq("username", input_user).execute()

        if not res.data:
            await interaction.response.send_message(
                "❌ Credenziali errate o username inesistente. Connessione rifiutata.",
                ephemeral=True
            )
            return
        ...
        if user_data.get("password_hash") != input_pass_hash:
            await interaction.response.send_message(
                "❌ Credenziali errate. Connessione rifiutata dal nodo.",
                ephemeral=True
            )
            return
        ...
        user = interaction.user
        user_discord_id = str(user.id)
        active_darkweb_sessions[input_user] = user_discord_id

        # Concede i permessi di visualizzazione sul canale Dark Web
        channel = interaction.guild.get_channel(DARKWEB_CHANNEL_ID)
        if channel:
            await channel.set_permissions(user, view_channel=True, send_messages=True)

        await interaction.response.send_message(
            f"🔒 **Connessione sicura stabilita come `{input_user}`.** Il canale Dark Web è ora sbloccato per te.",
            ephemeral=True
        )


# --- COMANDI DI CONFIGURAZIONE PANNELLI PERSISTENTI ---
@bot.tree.command(name="setup_darkweb_login", description="Invia il pannello di accesso persistente.")
async def setup_darkweb_login(interaction: discord.Interaction):
    view = DarkWebLoginPersistentView()
    await interaction.channel.send(
        "🌐 **TERMINALE TOR - ACCESSO DARK WEB**\n\n"
        "Clicca sul pulsante sottostante per autenticarti inserendo le tue credenziali.",
        view=view
    )
    await interaction.response.send_message("✅ Pannello di login avviato.", ephemeral=True)


@bot.tree.command(name="setup_darkweb_dashboard", description="Invia la dashboard persistente nel canale Dark Web.")
async def setup_darkweb_dashboard(interaction: discord.Interaction):
    if interaction.channel_id != DARKWEB_CHANNEL_ID:
        await interaction.response.send_message(f"❌ Questo comando va eseguito nel canale dedicato <#{DARKWEB_CHANNEL_ID}>.", ephemeral=True)
        return

    view = DarkWebDashboardPersistentView()
    await interaction.channel.send(
        "🥷 **PANNELLO DI CONTROLLO DARK WEB**\n\n"
        "Usa i bottoni sottostanti per navigare o disconnetterti in sicurezza.",
        view=view
    )
    await interaction.response.send_message("✅ Dashboard avviata.", ephemeral=True)


# --- MODAL PER IL MESSAGGIO ANONIMO ---
class AnonimoModal(ui.Modal, title="Invia Messaggio Crittografato"):
    def __init__(self, username: str):
        super().__init__()
        self.username = username

        self.messaggio = ui.TextInput(
            label="Messaggio",
            placeholder="Scrivi il messaggio in-character...",
            style=discord.TextStyle.paragraph,
            required=True,
            max_length=1000
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        testo = self.messaggio.value.strip()

        destinazione = interaction.client.get_channel(DARKWEB_CHANNEL_ID)
        if not destinazione:
...
# --- COMANDO /ANONIMO ---
@bot.tree.command(name="anonimo", description="Invia un messaggio anonimo sfruttando il tuo profilo Dark Web attivo.")
@app_commands.describe(username="Il tuo username anonimo del Dark Web")
async def anonimo(interaction: discord.Interaction, username: str):
    user_discord_id = str(interaction.user.id)
    ...
    await interaction.response.send_modal(AnonimoModal(username=target_username))


# ==========================================
# ⚠️ IMPORTANTE: DA AGGIUNGERE NELL'EVENTO on_ready() DEL BOT:
# bot.add_view(DarkWebLoginPersistentView())
# bot.add_view(DarkWebDashboardPersistentView())
# ==========================================
