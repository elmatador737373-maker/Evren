...
ID_RUOLO_EDILIZIA = 1549740475202670642
...
# ==========================================
TUO_ID_DISCORD = 1191824316376043580  # Inserisci il tuo ID Discord numerico reale

EVA_SUPABASE_URL = os.getenv("EVA_SUPABASE_URL")
EVA_SUPABASE_KEY = os.getenv("EVA_SUPABASE_KEY")
GROQ_API_KEY = os.getenv("GROQ_API_KEY")

# Client Supabase dedicato esclusivamente a E.V.A.
eva_supabase: Client = create_client(EVA_SUPABASE_URL, EVA_SUPABASE_KEY)

# Client Groq per l'Intelligenza Artificiale
groq_client = Groq(api_key=GROQ_API_KEY)


# ==========================================
# 💾 FUNZIONI DI MEMORIA (DATABASE INDIPENDENTE)
# ==========================================
def salva_memoria(user_id: str, informazione: str):
    """Archivia un nuovo fatto nel database isolato di E.V.A."""
    try:
        ...
        print(f"[E.V.A. MEMORY ERROR] Scrittura fallita: {e}")

def recupera_memorie(user_id: str) -> str:
    """Estrae i ricordi storici dell'utente dal database dedicato."""
    try:
        res = (
        ...
        print(f"[E.V.A. MEMORY ERROR] Lettura fallita: {e}")
        return "Archivio di memoria temporaneamente inaccessibile."


# ==========================================
# 📩 EVENTO MESSAGGI: RADAR OFFLINE & E.V.A.
# ==========================================
@bot.event
async def on_message(message: discord.Message):
    # Ignora i messaggi inviati dai bot per evitare loop
    if message.author.bot:
        return

    # ----------------------------------------------------
    # 1. RADAR DI STATO: CONTROLLO OFFLINE (SE TI TAGGANO)
    # ----------------------------------------------------
    if any(mention.id == TUO_ID_DISCORD for mention in message.mentions):
        if message.guild:
            ...
            if target_member:
                # Verifica se lo stato è offline o invisibile
                is_offline = target_member.status in [discord.Status.offline, discord.Status.invisible]
                ...
                if is_offline:
                    embed_offline = discord.Embed(
                        ...
                        color=discord.Color.from_rgb(30, 41, 59) # Slate scuro formale
                    )
                    embed_offline.set_footer(
                        text="Emerald OS • Terminale di Comunicazione", 
                        icon_url=message.guild.icon.url if message.guild.icon else None
                    )
                    await message.reply(embed=embed_offline, mention_author=False)
                # NOTA: Se sei ONLINE, il codice prosegue senza fare nulla, esattamente come richiesto.

    # ----------------------------------------------------
    # 2. E.V.A. - ASSISTENTE AI CHE IMPARA (SE TAGGANO IL BOT)
    # ----------------------------------------------------
    if bot.user in message.mentions:
        testo_pulito = message.content.replace(f"<@{bot.user.id}>", "").strip()
        user_id_str = str(message.author.id)

        # Se ti taggano senza scrivere nulla
        if not testo_pulito:
            embed_greet = discord.Embed(
                ...
                color=discord.Color.from_rgb(46, 204, 113) # Verde Smeraldo Emerald RP
            )
            await message.reply(embed=embed_greet, mention_author=False)
            return

        async with message.channel.typing():
            # Recupera lo storico dei ricordi dal DB separato di E.V.A.
...
Il tuo tono è elegante, analitico, estremamente cortese e formale (stile J.A.R.V.I.S.).
...
3. L'uso di [MEMORY: ...] è limitato solo alle informazioni che meritano di essere salvate nel database a lungo termine. Non usarlo per i saluti o le domande generiche.
"""
            ...
            try:
                ...
                # Analisi della risposta per estrarre eventuali memorie da salvare
                ...
                await message.reply(embed=embed_ai, mention_author=False)

                # Feedback visivo: aggiunge una reazione 💾 se ha imparato un nuovo dettaglio
                if memoria_salvata:
    ...
    # Permette al bot di continuare a elaborare tutti gli altri comandi (slash e prefisso)
